...
const OrderDetailPage = () => {
    ...
    useEffect(() => {
        const fetchOrder = async () => {
            try {
                ...
                setRevisions(response.data.revision || []); // Ensure revisions is always an array
                setLoading(false);
    ...
    const renderRevisionFiles = (files) => {
        ...
        return parsedFiles.map((file, idx) => {
            ...
            if (['mp3', 'wav', 'm4a'].includes(fileExtension)) {
                ...
                return <img key={idx} src={`${DOMAIN}${file}`} alt={`revision file ${idx + 1}`} className="w-full h-auto rounded-lg mt-2" />;
            } else {
                return <p key={idx} className="text-gray-400">Unsupported file type: {file}</p>;
            }
        });
    };
    ...
    const handleOnClickRead = async (itemId) => {
        ...
        try {
            const response = await axios(`${API_ENDPOINT}user-flag/${orderId}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Accept': 'application/json',
                                ...
                                <div className="w-full flex flex-col gap-5">
                                    ...
                                    {isAccordionOpen('orderInfo') && (
                                        <div className="p-8 flex flex-col gap-3 bg-black rounded-[20px] text-white">
                                            ...
                                            {isGiftCard == 1 && (
                                                <p className="font-THICCCBOI-Medium text-lg"><strong>Order Status:</strong><span className={`bg-[${ORDER_STATUS[orderStatus].color}] w-fit rounded-md px-2 py-1 mx-1`}>{ORDER_STATUS[orderStatus].name}</span></p>
                                            )}
                                            ...
                                            <p className="font-THICCCBOI-Medium text-lg"><strong>Date:</strong> {new Date(order.created_at).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</p>
                                                ...
                                                {orderItems.map((item) => (
                                                    <div key={item.id} className="p-6 flex flex-col gap-3 bg-black rounded-[20px] text-white relative"
                                                        ...
                                                        <div className='flex flex-col md:flex-row gap-5'>
                                                            ...
                                                            {item?.deliverable_files && JSON.parse(item?.deliverable_files)?.length > 0 && (
